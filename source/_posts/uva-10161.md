...
棋盘上的蚂蚁（UVa 10161）这道题应该算是找规律的题吧。虽然理论上可以 1, 2, 3, 4... 地推下去，但数据范围比较大（{% math %}1 \leq N \leq 2 \times 10^9{% endmath %}），我觉得这样可能会超时。所以还是决定去找规律。
...
对于棋盘上的每一个数（记为 _N_ ），我们将它的平方根向上取整（{% math %}\lceil\sqrt{N}\rceil{% endmath %}，记为 _Key_ ），可以看到：
1. 当 _Key_ 为奇数时，_Key_{% math %}^2{% endmath %} 在第一纵列上（黄底色）。
2. 当 _Key_ 为偶数时，_Key_{% math %}^2{% endmath %} 在第一橫行上（绿底色）。
...
例如要求 N 的坐标：
{% math %} N=27, Key=\lceil\sqrt{27}\rceil=6, Point=6\times(6-1)+1=31, c=27-31=-4{% endmath %}
